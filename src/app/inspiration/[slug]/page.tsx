    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        <Link href="/inspiration" className="inline-flex items-center text-sm text-slate-500 hover:text-slate-900 mb-4">
          <ArrowLeft className="h-4 w-4 mr-1" /> All case studies
        </Link>
        <Badge className="mb-3">{study.category === "ai_agent" ? "AI Agent Solution" : "Vibe Coding Startup"}</Badge>
        <h1 className="text-2xl font-bold mb-2">{study.title}</h1>
        <p className="text-slate-600 mb-4">{study.summary}</p>
        <div className="flex flex-wrap gap-2 mb-8">
          {study.tech_stack.map((t) => <Badge key={t} variant="outline">{t}</Badge>)}
        </div>
        <Card>
